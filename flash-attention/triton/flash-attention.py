...
def _attn_fwd(
    ...
    if STAGE == 3:
        # this step runs for the blocks to the right of the diagonal in the causal attnetion
        O_block, l_i, m_i = _attn_fwd_inner(
            O_block,
            l_i,
            m_i,
...
def _attn_bwd_dq(
    ...
    # Make sure pointers are in the right place w.r.t batch and the head
    ...
    for blk_idx in range (num_steps):
        ...
        # Compute dQ.
        # NOTE: We need to de-scale dq in the end, becasue kT was pre-scaled
...
def _attn_bwd_dk_dv(
    ...
    # We access the Q as transposed array, so that's why we threat offs_q as a column vector and offs_dim as a row_vector
...
class TritonAttention(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, causal, softmax_scale):
        ...
        # Number of parallel programs: (BATCH_SIZE * NUM_HEADS) * ceil(SEQ_LEN / BLOCK_SIZE_Q)
    ...
    def backward(ctx, dO):
        ...
        _attn_bwd_preprocess[preprocessing_grid](
            O=O,
            dO=dO,
            D=D,
            SEQ_LEN = SEQ_LEN,
        ...
        # Fix KV and integrate through all the Q blocks
